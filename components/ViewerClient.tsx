...
export default function ViewerClient({ deviceId, password }: Props) {
  ...
  useEffect(() => {
    ...
    const authenticate = async () => {
      ...
      const result = await new Promise<{ ok: boolean; error?: string; device?: { deviceName?: string } }>((resolve) => {
        socket.emit(
          'viewer:authenticate',
          {
            deviceId,
    ...
    socket.on('webrtc:offer', async ({ offer }) => {
      try {
        ...
        setStatus('error');
        setError(peerError instanceof Error ? peerError.message : 'Failed to handle offer');
      }
    });
  ...
  return (
    <main style={{ minHeight: '100vh', padding: 24 }}>
      ...
      >
        ...
        <p style={{ color: status === 'error' ? 'var(--danger)' : 'var(--accent)' }}>
          Status:
          {' '}
          {status}
          {error ? ` - ${error}` : ''}
